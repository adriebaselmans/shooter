...
public sealed class MetalRenderBackend : IRenderBackend
{
    ...
    private unsafe void RunScenePass(IntPtr commandBuffer, RenderFrameData frame, FrameMatrices m)
    {
        ...
        if (_skyPipeline != IntPtr.Zero)
        {
            var sky = BuildSkyUniforms(frame);
            MsgVoid(encoder, "setRenderPipelineState:", _skyPipeline);
            MsgVoid(encoder, "setDepthStencilState:", _depthStateDisabled);
            SetFragmentBytes(encoder, ref sky, 0UL);
        ...
        if (_depthStateWriteLess != IntPtr.Zero) MsgVoid(encoder, "setDepthStencilState:", _depthStateWriteLess);
        MsgVoid(encoder, "setVertexBuffer:offset:atIndex:", _vertexBuffer, 0UL, 0UL);
        MsgVoid(encoder, "setFragmentSamplerState:atIndex:", _linearRepeatSampler, 0UL);
    ...
    private SkyUniforms BuildSkyUniforms(RenderFrameData frame)
    {
        var (zenith, horizon) = BuildSkyColors(frame);
        float aspect = _fbHeight > 0 ? (float)_fbWidth / _fbHeight : 16f / 9f;
        float tanHalfFov = MathF.Tan(RenderConstants.ViewmodelFovYRadians * 0.5f);
        Vector3 forward = Vector3.Normalize(frame.Player.Forward());
        Vector3 right = Vector3.Normalize(frame.Player.Right());
        Vector3 up = Vector3.Normalize(Vector3.Cross(right, forward));
        return new SkyUniforms
        {
            CameraForward = new Vector4(forward, 0f),
            CameraRight = new Vector4(right, 0f),
    ...
    private IntPtr CreateSceneRenderPass()
    {
        ...
        MsgVoid(color0, "setClearColor:", MetalNative.CreateClearColor(0.0, 0.0, 0.0, 1.0));
...
float3 skyDirection(float2 uv, constant SkyUniforms& u){
    float2 ndc = float2(uv.x * 2.0 - 1.0, 1.0 - uv.y * 2.0);
    float3 dir = u.cameraForward.xyz
               + u.cameraRight.xyz * (ndc.x * u.projInfo.x)
               + u.cameraUp.xyz * (ndc.y * u.projInfo.y);
...
fragment SceneOut fs_sky(FsQuadOut in [[stage_in]], constant SkyUniforms& u [[buffer(0)]]) {
    SceneOut o;
    float3 dir = skyDirection(in.uv, u);
    float3 sky = analyticSky(dir, normalize(u.toSun.xyz), u.sunColor.xyz, u.skyZenith.xyz, u.skyHorizon.xyz, u.groundColor.xyz, u.projInfo.z);
    o.color = half4(float4(sky, 1.0));
    o.normal = half4(0.0);
    o.albedo = half4(0.0);
    return o;
}
...
fragment SceneOut fs_scene(SceneVsOut in [[stage_in]],
    ...
    SceneOut o;
    float4 base = tex.sample(repeatSamp, in.uv) * in.color;
    ...
    base = tex.sample(repeatSamp, sampleUv) * in.color;
    float3 sampledNormal = normalTex.sample(repeatSamp, sampleUv).xyz * 2.0 - 1.0;
    float3 n = dot(sampledNormal, sampledNormal) > 0.2
        ? normalize(cotangentFrame(geomN, in.worldPos, sampleUv) * sampledNormal)
        : detailNormalFromAlbedo(tex, repeatSamp, sampleUv, geomN, draw.materialParams.z);
    float roughness = roughTex.sample(repeatSamp, sampleUv).r;
...
""";
        ...
        _skyPipeline = CreateRenderPipeline(library, "vs_fullscreen", "fs_sky", PixelFormatRgba16Float, PixelFormatRgba16Float, PixelFormatRgba16Float, PixelFormatDepth32Float, blendMode: BlendMode.Disabled);
